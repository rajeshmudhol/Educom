import React from 'react'
import "./chain.css"

function Chain() {
  return (
    <div className="chbachg">
        <div className="ana">
        <div className="sub-chain">
<h1>Blockchain Developer</h1>
<i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>
            
<p>This Blockchain basics program will help you cut through the buzz surrounding blockchain,Bitcoin, </p><br></br>
<p>and cryptocurrency. Learn blockchain and its core platforms While mastering important tools like</p><br></br>
<p>Ganache.Truffe.MetaMask.and Geth. you will also be  able to create apps with Ethereum and set up a small private networl</p><br></br>
<h4>17306 Ratings </h4>

<h5>52707 Learns</h5>
</div>
<div className="chain-skill">
    <h2>Skills you will gain</h2>
    <p>Blockchain overview</p>
    <p>Bitcoin Blockchain</p>
    <p>Ethereum Deployment tools</p>
    <p>Hyperledger Composer</p>
    <p>Smart contract on Multichain</p>
    <p>Blockchain on Multichain </p>
    <p>Blockchain prospect</p>
    


        </div>
        <div className="chain-lear">
            <h2>Who should learn</h2>
            <p id="chde">Developers</p>
            <p id="chpm">Project Managers</p>
            <p id="chsa">Solution Architects</p>
            <p id="chsp">Support Professionals</p>
            <p id="chtl">Team Leads</p>
            <p id="chpm">Product Managers</p>
            <p id="chba">Business Analysts</p>
        </div>
        <div className="chprojct">
          <h2>Industry Project</h2>
          <div className="container15">
            <div className="chain-projc">
              <h5>Project 1</h5>
              <h3>Voting with Blochain</h3>
              <p>You have asked to create and deploy an</p>
              <p>application that can overcome the abuse of </p>
              <p>electrion voting framework by leveraging</p>
              <p>needs of customers in the education</p>
              
              {/* <img src="https://mma.prnewswire.com/media/617186/Pearson_Logo.jpg?p=twitter"></img> */}

            </div>
            <div className="chain-projc">
              <h5>Project 2</h5>
              <h3>Ensuring Transparency in Trade Finance Transactions</h3>
              <p>You are tasked with creating an application </p>
              <p>that replaces the paper-based framework,</p>
              <p>preserving digital list of exchanges, and </p>

              
            </div>
            <div className="chain-projc">
              <h5>Project 3</h5>
              <h3>PGA Golf is Sunday Made For Tv</h3>
              <p>Analyse a Golf tournment to</p>
              <p>identify if Courses were made easler on </p>
              <p>sunday to telecast low scores or if the </p>
              <p>pressure on the golfers raised their scores</p>
              <p></p>
             

              
            </div>
          </div>


        </div>
        <div className="chreviws">
          <h2>What other Learners are saying</h2>
          <div className="container16">

            <div className="chain-revu">
              <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRxhRCoaB3ZF4oPSW78AIjBJZIKS61ZsF7IWxuNHyFIzCmcTzmo0AAXzrh8hbu9XL6-W08&usqp=CAU"></img>
              <h3>Shekhar Lele </h3>
              <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>
                <p>The certification helped me with my career.After finishing the</p>
                <p>course,i was assigned new projects with in my organization.my</p>
                

            </div>
            <div className="chain-revu">
              <img src="data:image/jpeg;base64,"></img>
              <h3>sanjay </h3>
              <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>

                <i class="fas fa-star"></i>
                <p>The instructor was very knowledgeable and provided live project</p>
                <p>cases.making the sessions more enjoyable.The topics were made</p>
                <p>easy-to-understand with real-life examples.</p>
                {/* <p></p>
                <p></p> */}
                

              
            </div>
          </div>
        </div>
        </div>
        </div>

  )
}

export default Chain;

